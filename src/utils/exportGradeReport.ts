...
export async function exportGradeReportToExcel(
  ...
  const studentMap = new Map<
    string,
    { studentCode: string; studentName: string; reports: GradeReportData[] }
  >();
  ...
  for (const [studentId, studentData] of studentMap.entries()) {
    ...
    for (const [courseElementName, courseElementReports] of courseElementMap.entries()) {
      ...
      const submittedAt =
        latestSubmission && latestSubmission.submittedAt && latestSubmission.submittedAt !== ''
        ...
        for (const question of questions) {
          ...
          if (questionRubrics.length === 0) {
            ...
            for (const rubric of questionRubrics) {
              ...
              const gradeItem = allGradeItems.find((item) => item.rubricItemId === rubric.id);
  ...
  try {
    ...
    const path = `${RNBlobUtil.fs.dirs.DownloadDir}/${finalFileName}`;
